...
from ipnet.imitation.behavioural_cloning import get_bc_agent_from_saved, get_meta_bc_agent_from_saved
...
def my_config():
    ...
    # Choice among: bc_train, bc_test, sp, hm, rnd, meta_bc_train
    OTHER_AGENT_TYPE = "meta_bc_train"
...
def configure_other_agent(params, gym_env, mlp, mdp, bc_model_path = None):
    ...
    elif params["OTHER_AGENT_TYPE"] == "meta_bc_train": 
        agent, bc_params = get_meta_bc_agent_from_saved(bc_model_path)
        gym_env.use_action_method = True
        # Make sure environment params are the same in PPO as in the BC model
        for k, v in bc_params["env_params"].items():
            assert v == params["env_params"][k], "{} did not match. env_params: {} \t PPO params: {}".format(k, v, params[k])
...
def ppo_run(params):
    ...
    for seed in params["SEEDS"]:
        ...
        # Get paths for BC agents
        bc_dir = os.path.join(CHECKPOINT_DIR, 'meta_bc_runs')
        bc_agent_dirs = filter_train_agents(bc_dir, params["mdp_params"]['layout_name'])
        
        # Path for pre-trained PPO_IP - BC model
        ppo_model_path = os.path.join(CHECKPOINT_DIR,
        ...
        for bc_agent_dir in bc_agent_dirs:
            ...
            configure_other_agent(params, gym_env, mlp, mdp, bc_model_path=bc_agent_dir) # configure bc agent
