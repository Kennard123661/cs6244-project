...
class AgentPair(AgentGroup):
    ...
    def __init__(self, *agents, is_ipnet_first: bool, allow_duplicate_agents=False):
        super().__init__(*agents, allow_duplicate_agents=allow_duplicate_agents)
        assert self.n == 2
        self.a0, self.a1 = self.agents
        self.is_ipnet_first = is_ipnet_first
...
class AgentFromPolicy(Agent):
    ...
    def __init__(self, state_policy, direct_policy, sequence_length, stochastic=True, action_probs=False):
        """
    ...
    def action(self, state):
        ...
        try:
            if len(self.history) >= self.sequence_length:
                observations = self.history[-self.sequence_length:]
            else:
                num_padding = self.sequence_length - len(self.history)
                observations = [self.history[0]] * num_padding + self.history
            observations = np.array(observations)
            return self.state_policy(observations, self.mdp, self.agent_index, self.stochastic, self.action_probs)
