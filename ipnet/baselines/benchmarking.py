...
DEFAULT_TRAJ_KEYS = ["ep_observations", "ep_actions", "ep_rewards", "ep_dones", "ep_returns", "ep_returns_sparse",
                     "ep_lengths", "mdp_params", "env_params"]
...
class AgentEvaluator(object):
    ...
    def __init__(self, mdp_params, env_params={}, mdp_fn_params=None, force_compute=False,
                 mlp_params=NO_COUNTERS_PARAMS, debug=False):
        """
    ...
    def _check_standard_traj_keys(traj_keys_set):
        assert traj_keys_set == set(
            DEFAULT_TRAJ_KEYS), "Keys of traj dict did not match standard form.\nMissing keys: {}\nAdditional keys: {}".format(
    ...
    def _check_right_types(trajectories):
        for idx in range(len(trajectories["ep_observations"])):
            states, actions, rewards = trajectories["ep_observations"][idx], trajectories["ep_actions"][idx], \
    ...
    def save_traj_as_json(trajectory, filename):
        ...
        dict_traj["ep_observations"] = [[ob.to_dict() for ob in one_ep_obs] for one_ep_obs in
                                        trajectory["ep_observations"]]
    ...
    def load_traj_from_json(filename):
        ...
        traj_dict["ep_observations"] = [[OvercookedState.from_dict(ob) for ob in curr_ep_obs] for curr_ep_obs in
                                        traj_dict["ep_observations"]]
        traj_dict["ep_actions"] = [[tuple(tuple(a) if type(a) is list else a for a in j_a) for j_a in ep_acts] for
                                   ep_acts in traj_dict["ep_actions"]]
        return traj_dict
    ...
    def interactive_from_traj(trajectories, traj_idx=0):
        ...
        def update(t=1.0):
            ...
            print(env)
            print("Joint Action: {} \t Score: {}".format(Action.joint_action_to_char(joint_action),
