...
from ipnet.baselines.benchmarking import AgentEvaluator
...
from ipnet.ppo.ppo import get_ppo_agent, plot_ppo_run, IPNET_PPO_DATA_DIR

# todo: make this consistent with your history length
HISTORY_LENGTH = 10
...
def evaluate_ppo_and_bc_models_for_layout(layout, num_rounds, bc_model_paths, ppo_bc_model_paths, seeds, best=False, display=False):
    ...
    for seed_idx in range(len(seeds["bc_train"])):
        ...
        ppo_and_bc = evaluator.evaluate_agent_pair(AgentPair(agent_ppo_bc_train, agent_bc_test, is_ipnet_first=True),
                                                   num_games=num_rounds, display=display)
        avg_ppo_and_bc = np.mean(ppo_and_bc['ep_returns'])
        ppo_bc_performance[layout]["PPO_BC_train+BC_test_0"].append(avg_ppo_and_bc)

        bc_and_ppo = evaluator.evaluate_agent_pair(AgentPair(agent_bc_test, agent_ppo_bc_train, is_ipnet_first=False),
                                                   num_games=num_rounds, display=display)
        ...
        ppo_and_bc = evaluator.evaluate_agent_pair(AgentPair(agent_ppo_bc_test, agent_bc_test,
                                                             is_ipnet_first=True), num_games=num_rounds, display=display)
        avg_ppo_and_bc = np.mean(ppo_and_bc['ep_returns'])
        ppo_bc_performance[layout]["PPO_BC_test+BC_test_0"].append(avg_ppo_and_bc)

        bc_and_ppo = evaluator.evaluate_agent_pair(AgentPair(agent_bc_test, agent_ppo_bc_test, is_ipnet_first=False),
                                                   num_games=num_rounds, display=display)
        avg_bc_and_ppo = np.mean(bc_and_ppo['ep_returns'])
...
def run_all_ppo_bc_experiments(best_bc_model_paths):
    ...
    # todo: I haven't fixed the plotting training curves part yet.
    # plot_runs_training_curves(ppo_bc_model_paths, seeds, save=True)

    set_global_seed(248)

    # todo: change back num_rounds to 100
    # num_rounds = 100
    num_rounds = 1
    ...
    save_pickle(ppo_bc_performance, IPNET_PPO_DATA_DIR + "ppo_bc_models_performance")


if __name__ == '__main__':
    import os
    best_path = os.path.join(BC_SAVE_DIR, 'best_bc_model_paths')
    best_paths = load_pickle(best_path)
    run_all_ppo_bc_experiments(best_bc_model_paths=best_paths)
