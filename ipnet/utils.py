...
def conv_network_fn(**kwargs):
    ...
    def hypernetwork_heads(_inputs, out_shape: list):
        """
        Predicts the weights for a single linear layer.

        Args:
            _inputs:  B x D inputs
            out_shape:  [policy_in, policy_out] where r is the rows of the weight matrix and B is the columns of
            weight matrix

        Returns:
            Pin x Pout matrix and bias
        """
        batchsize = _inputs.shape[0]
        assert len(out_shape) == 2
        policy_in, policy_out = out_shape
        weight_dims = policy_in * policy_out

        out = _inputs
        for _ in range(num_hypernet_layers):
            out = tf.layers.dense(out, size_hidden_layers, activation=tf.nn.leaky_relu)
        weight = tf.layers.dense(out, weight_dims, activation=None)
        bias = tf.layers.dense(out, policy_out, activation=None)

        weight = tf.reshape(weight, shape=[batchsize, policy_in, policy_out])
        return weight, bias

    def hypernetwork_generator(_inputs, _policy_in_dims: int) -> (list, list):
        """
        Args:
            _inputs: [B, L, H, W, D] where B is the batch size, L is the sequence length of inputs
            _policy_in_dims: integer of the input

        Returns:
            returns a list of weight matrices and biases
        """
        assert len(_inputs.shape) == 5
        batchsize, h_length, height, width, depth = _inputs.shape
        out = tf.reshape(_inputs, shape=[batchsize * h_length, height, width, depth])  # BL x H x W x D
        out = conv_fn(conv_in=out)  # BL x Dout
        out_dims = tf.shape(out)[1]
        out = tf.reshape(out, shape=[batchsize, h_length, out_dims])

        out = tf.reshape(out, shape=[batchsize, h_length * out_dims])
        weights, biases = [], []
        weight, bias = hypernetwork_heads(_inputs=out, out_shape=[_policy_in_dims, size_hidden_layers])
        weights.append(weight)
        biases.append(bias)
        for _ in range(num_hidden_layers-1):
            weight, bias = hypernetwork_heads(_inputs=out, out_shape=[size_hidden_layers, size_hidden_layers])
    ...
    def network_fn(_x):
        """
        Takes _x as inputs, predict weights for the linear layer of the policy network using a hypernetwork,
        then compute the output embedding

        Args:
            _x: B x T x H x W x D

        Returns:
            B x D latent vectors
        """
        assert len(_x.shape) == 5

        _inputs = _x[:, history_length - 1, :, :, :]
