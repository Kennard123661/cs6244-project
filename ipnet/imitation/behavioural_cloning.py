...
from human_aware_rl.human.process_dataframes import save_npz_file, get_worker_trajs_from_data
from human_aware_rl.directory import CHECKPOINT_DIR, DATA_DIR

BC_SAVE_DIR = os.path.join(CHECKPOINT_DIR, 'meta_bc_runs' + os.path.sep)
...
def train_meta_bc_agents(model_save_dir, bc_params, num_epochs=1000, lr=1e-4, adam_eps=1e-8) -> dict:
    ...
    for worker, trajs in expert_trajs.items(): 				# for keys, trajectories in the dictionary 'expert_trajs'
        ...
        with tf.variable_scope('worker-{}'.format(worker), reuse=False):
            bc_model = bc_from_dataset_and_params(dataset, bc_params, save_dir, num_epochs, lr, adam_eps)
        model_dict[worker] = bc_model
    return model_dict
